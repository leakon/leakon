...
class SofavDB_Manager {
	...
	public static function getConf($connectionName = 'Table') {
		...
		if (isset(self::$arrDataSources[$connectionName])) {
			...
			// example	mysql://root:123456@localhost:3306/db?encoding=utf8&persistent=on&debug=on
		...
		foreach (explode("\n", $strDataSource) as $strItem) {
			...
			if (strlen($strItem)) {
				...
				/*
					...
					first:	mysql://pma@localhost:3306/db?encoding=utf8&persistent=off	# test comment

					# The first connection
					second: mysql://root:123456@localhost:3306/db?encoding=utf8&persistent=on
				*/
